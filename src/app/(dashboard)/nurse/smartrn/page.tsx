...
export default function SmartRNPage() {
  ...
  return (
    <div className="flex flex-col bg-brand-warm">
      <div className="mx-auto flex w-full max-w-3xl flex-1 flex-col px-4 py-6">
        {messages.length === 0 && !loading && (
          <div className="flex flex-col items-center gap-2 pt-6 pb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-2xl font-bold text-brand-charcoal">SmartRN</h1>
                <p className="text-sm text-brand-gray-500">Powered by real nurse community data</p>
              </div>
            </div>
            <p className="max-w-md text-center text-brand-gray-500">
              Ask me anything about travel nursing — pay rates, stipends, contracts, negotiation, housing, and more.
            </p>
          </div>
        )}
          ...
          <div className="flex-1 space-y-4 pb-4">
            {messages.map((msg, i) => (
              <div key={i} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
                <div className={`max-w-[85%] ${msg.role === "user" ? "rounded-2xl rounded-br-md bg-brand-orange px-4 py-3 text-white" : "space-y-2"}`}>
                  {msg.role === "model" && (
                    <div className="flex items-start gap-2">
                      ...
                      <div className="rounded-2xl rounded-bl-md bg-white px-4 py-3 shadow-sm">
                        <div className="prose prose-sm max-w-none text-brand-charcoal whitespace-pre-wrap">{msg.content}</div>
                        {msg.sources && msg.sources.length > 0 && (
                          <div className="mt-3 border-t border-brand-gray-100 pt-2">
                            <button onClick={() => toggleSources(i)} className="flex items-center gap-1 text-xs text-brand-gray-400 hover:text-brand-gray-600">
                            ...
                            {expandedSources.has(i) && (
                              <div className="mt-2 space-y-2">
                                {msg.sources.map((src, si) => (
                                  <div key={si} className="rounded-lg bg-brand-gray-50 p-2 text-xs">
                                    <p className="font-medium text-brand-charcoal">{src.question}</p>
                                    <p className="mt-1 text-brand-gray-500">{src.answer_snippet}</p>
        ...
        <div className="sticky bottom-0 border-t border-brand-gray-200 bg-brand-warm pt-4 pb-4">
          ...
          <p className="mt-2 text-center text-[11px] text-brand-gray-400">
            SmartRN answers are based on community data and may not be fully accurate. Always verify with the facility.
          </p>
          {messages.length === 0 && !loading && (
